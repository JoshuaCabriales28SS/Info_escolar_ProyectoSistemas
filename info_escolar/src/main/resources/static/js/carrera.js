    ...
    const parsearRespuesta = (data) => {
        const lista = data?._embedded?.results ?? (Array.isArray(data) ? data : []);
    ...
    const buscar = () => {
        ...
        fetch('http://localhost:8080/car_api', { credentials: 'include' })
            .then(res => { if (!res.ok) throw new Error(`HTTP ${res.status}`); return res.json(); })
            .then(data => { _cache = parsearRespuesta(data); filtrar(); })
            .catch(() => {
                fetch('https://ec.europa.eu/esco/api/search?type=occupation&text=developer&language=es')
                    .then(r => r.json())
                    .then(data => { _cache = parsearRespuesta(data); filtrar(); })
                    .catch(e => {
